...
with st.sidebar:
    st.header('🔍 Filters')
    start_station = st.selectbox('Start Station', ['All'] + sorted(df['start_station_name'].dropna().unique()))
    end_station = st.selectbox('End Station', ['All'] + sorted(df['end_station_name'].dropna().unique()))
    start_neighborhood = st.selectbox('Start Station Neighborhood', ['All'] + sorted(df['start_station_neighborhood'].dropna().unique()))
    end_neighborhood = st.selectbox('End Station Neighborhood', ['All'] + sorted(df['end_station_neighborhood'].dropna().unique()))
    membership_types = st.multiselect('Membership Types', sorted(df['product_name'].dropna().unique()))
    selected_years = st.multiselect('Year(s)', sorted(df['year'].dropna().unique()))
    selected_months = st.multiselect('Month(s)', sorted(df['month'].dropna().unique()))
    pitt_filter = st.radio('Include Pitt Riders', ['All', 'Only Pitt Riders', 'Exclude Pitt Riders'])
